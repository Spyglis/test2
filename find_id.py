...
def find_new_pattern(read_text, idx_list, pattern, path): # todo manage function in function by counting white spacec before def?
...
def find_function_name(path, find_pattern):
    ...
    for idx, text in enumerate(read_text):
        if text.find(find_pattern) != -1:
            idx_list.append(idx)
    return find_new_pattern(read_text=read_text, idx_list=idx_list, pattern=find_pattern, path=path)
...
def start_function(source_path: Path, pattern: str) -> list:
    ...
    for i in range(6):
        ...
        for pat, value in pattern_dict.items():
            if value == 0:            # 
                print(pat)
                id_list, path_list = find_tests(source_path=source_path, find_pattern=pat)
